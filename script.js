...
async function fetchDataAndInitialize() {
    try {
        ...
        if (!Array.isArray(allBrandData) || allBrandData.length === 0) { throw new Error('Invalid data format.'); }
        console.log("Data loaded successfully.");
        renderPage();
    } catch (error) {
        console.error("Fetch/Parse Error:", error);
        const contentDiv = document.getElementById('content');
        if(contentDiv) contentDiv.innerHTML = `<p style="color:red; text-align:center;">Error loading data. Check console.</p>`;
    }
}
...
function renderPage() {
    ...
    try {
        if (brandNameParam) {
            const decodedBrandName = decodeURIComponent(brandNameParam);
            renderBrandPage(decodedBrandName);
        } else {
...
function renderBrandPage(brandName) {
    ...
    const contentDiv = document.getElementById('content');
    const pageUrl = window.location.pathname;
    if (!brandData) { contentDiv.innerHTML = `<h1>Brand Not Found</h1><p><a href="${pageUrl}" class="back-link">Back to list</a></p>`; return; }
    ...
    try {
        contentDiv.innerHTML = `
            <a href="${pageUrl}" class="back-link">&larr; Back to Brand List</a>
            ...
            <div class="brand-page-layout">
                <div class="brand-main-content-wrapper">
                    <div class="promo-banner" style="background-image: url('http://passby.com/wp-content/uploads/2025/10/Artboard-scaled.jpg');" onclick="document.getElementById('custom-html-card').scrollIntoView({ behavior: 'smooth' });">
                         <p>Unlock the full story behind ${brandData.brand_name}'s performance with <a href="https_passby.com" target="_blank">Pass_by Pro</a>. See causal drivers, competitor benchmarks, and more.</p>
                    </div>
